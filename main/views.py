...
from main.utils import is_json, convert_into_array
from main.ml_code import find_symptoms
...
def predict(request):
    if request.method == 'POST':
        data = request.body
        if not is_json(data):
            json_data = json.dumps({'status': "failed", "error": "Invalid Json"})
            return JsonResponse(json_data, status=400)
        data = json.loads(data)
        data = convert_into_array(data.get('data'))
        result = list(find_symptoms(data))
        context = {
            "status": "success",
            "data": {
                "predicted": list(result[0]),
                "confidencescore": format(result[1], '.0f')
            }
        }
        return JsonResponse(context, safe=False)

    symptoms = sorted(get_symptomslist())
    context = {
        "status": "success",
        "data": symptoms
    }
